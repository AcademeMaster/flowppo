...
class QC_FQLAgent(nn.Module):
    ...
    def train_step(self, batch, offline_mode=False):
        ...
        # ===== 3. 修复：增强的行为约束机制（动作块级约束）=====
        distill_loss = torch.tensor(0.0, device=self.device)
        if self.actor_type == "distill" and hasattr(self, 'actor_net') and self.actor_net is not None:
            # 生成目标动作（来自best-of-N或流采样）
            with torch.no_grad():
                target_actions = self.sample_actions(obs, strategy="best-of-n")  # 强制使用best-of-N
            ...
            # 修复3: 动作块级Wasserstein约束 (替代论文Eq.8的KL约束)
            # 使用L2范数作为Wasserstein距离的近似
            chunk_wise_distance = torch.norm(actor_actions - target_actions_normalized, p=2, dim=1)  # [B]
            distill_loss = torch.mean(chunk_wise_distance)  # 标量
